...
class D2LActivityExemptions extends mixinBehaviors([
  ...
  static get template() {
	return html`
		...
		<div role="main">
			<d2l-button class="toggle-exemption-buttons" role="button" aria-label$="[[localize('ariaExempt')]]" primary="" on-click="exemptSelected">
				[[localize('btnExempt')]]
			</d2l-button>

			<d2l-button class="toggle-exemption-buttons" role="button" aria-label$="[[localize('ariaUnexempt')]]" on-click="unexemptSelected">
			...
			<d2l-table id="classlist" role="grid" summary="[[localize('ariaTableSummary')]]" sticky-headers="">
				...
				<d2l-thead>
					<d2l-tr role="row">
						<d2l-th>
							<d2l-input-checkbox aria-label$="[[localize('selectUnselectAll')]]" on-change="selectAll">
							</d2l-input-checkbox>
						</d2l-th>
				...
				<d2l-tbody>
					<template is="dom-repeat" items="[[userData]]" observe="IsExempt">
						...
						<d2l-tr class="row-user" role="row" data="[[item]]">
							...
							<d2l-td>
								<d2l-input-checkbox class="checkbox-user" aria-label$="[[localize('ariaSelectUser', 'lastName', item.LastName, 'firstName', item.FirstName)]]">
								</d2l-input-checkbox>
							</d2l-td>
			...
			<template is="dom-if" if="[[hasMoreItems]]">
				<d2l-button class="toggle-exemption-buttons" role="button" aria-label$="[[localize('ariabtnLoadMore')]]" on-click="loadMore">
...
`;
  ...
  selectAll(e) {
	  dom(this.root).querySelectorAll('.checkbox-user')
		  .forEach( element => {
